...
def test_manager():
        ...
        assert "ossec: output: 'du -a /check_captured_command/ls_test_dir/': 4\\t/check_captured_command/ls_test_dir/cat_file.txt" in f.read()
        f.close()
        f = open('/var/ossec/logs/archives/archives.json', 'r')
        assert "ossec: output: 'du -a /check_captured_command/ls_test_dir/': 4\\t/check_captured_command/ls_test_dir/" in f.read()
        f.close()
        f = open('/var/ossec/logs/archives/archives.json', 'r')
